...
const { readFont, readImage } = require("../asset");
...
const avatar = readImage("avatar.jpg");
...
const getIcon = (name, color) => {
  ...
  return `
  <svg role="img" class="logo" fill="${fill}" style="filter:${filter};" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
const getHTML = (props) => {
  ...
  const githubColor = theme === "light" ? "#181717" : "white";
  ...
  <body>
    ...
    <main>
      <div class="logo-wrapper">
        ${icons
          .map((icon, i) => getIcon(icon, colors[i]))
          .filter((icon) => !!icon)
